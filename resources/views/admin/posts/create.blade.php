    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Write and prepare a new article.</p>
            </div>
                ...
                <div class="grid gap-8 lg:grid-cols-[18rem_1fr]">
                    <aside class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="published_at" name="published_at" type="datetime-local" class="mt-1 block w-full" :value="old('published_at')" data-publish-date-input />
                        ...
                        <div x-show="postType === 'article' || postType === 'gallery'" x-cloak>
                            ...
                            <input id="featured_image" name="featured_image" type="file" accept="image/*" class="mt-1 block w-full text-sm text-gray-700 file:mb-2 file:mr-4 file:rounded-md file:border-0 file:bg-gray-100 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-gray-700 hover:file:bg-gray-200 dark:text-gray-200">
                        ...
                        <div>
                            ...
                            <select id="category_id" name="category_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
                                ...
                                @foreach ($categories as $category)
                                    @include('admin.posts.partials.category-option', ['category' => $category, 'selectedCategoryId' => old('category_id')])
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="advertorial_id" name="advertorial_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
                                ...
                                @foreach ($advertorials as $advertorial)
                                    <option value="{{ $advertorial->id }}" @selected((string) old('advertorial_id') === (string) $advertorial->id)>
                        ...
                        <div>
                            ...
                            <select id="tags" name="tags[]" multiple data-select2-tags class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
                                @foreach ($tags as $tag)
                                    <option value="{{ $tag->id }}" @selected(in_array((string) $tag->id, old('tags', []), true))>{{ $tag->name }}</option>
                                @endforeach
                    ...
                    <main class="space-y-6">
                        ...
                        <div>
                            @if (auth()->user()->isEditor())
                                <x-input-label for="manual_author_names" value="Additional Writers" />
                                <textarea id="manual_author_names" name="manual_author_names" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" placeholder="Separate names with commas or new lines">{{ old('manual_author_names') }}</textarea>
                                <p class="mt-2 text-sm text-gray-500">Your name will be recorded automatically and cannot be removed from this post.</p>
                                <x-input-error :messages="$errors->get('manual_author_names')" class="mt-2" />
                            @else
                                <div class="rounded-md border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                                    Writer: {{ auth()->user()->name }} will be recorded automatically.
                                </div>
                            @endif
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="excerpt" name="excerpt" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">{{ old('excerpt') }}</textarea>
                            <x-input-error :messages="$errors->get('excerpt')" class="mt-2" />
                        </div>

                        <div x-show="postType === 'video'" x-cloak>
                            <x-input-label for="youtube_url" value="YouTube Link" />
                            <x-text-input id="youtube_url" name="youtube_url" type="url" class="mt-1 block w-full" :value="old('youtube_url')" placeholder="https://www.youtube.com/watch?v=..." />
                        ...
                        <div x-show="postType === 'gallery'" x-cloak>
                            ...
                            <input id="gallery_images" name="gallery_images[]" type="file" accept="image/*" multiple class="mt-1 block w-full text-sm text-gray-700 file:mr-4 file:rounded-md file:border-0 file:bg-gray-100 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-gray-700 hover:file:bg-gray-200 dark:text-gray-200">
                        ...
                        <div x-show="postType === 'infographic'" x-cloak>
                            ...
                            <input id="infographic_image" name="infographic_image" type="file" accept="image/*" class="mt-1 block w-full text-sm text-gray-700 file:mr-4 file:rounded-md file:border-0 file:bg-gray-100 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-gray-700 hover:file:bg-gray-200 dark:text-gray-200">
                        ...
                        <div>
                            ...
                            <div data-quill-editor data-quill-input="#body" class="mt-1 min-h-[28rem] rounded-b-md bg-white dark:bg-gray-900">{!! old('body') !!}</div>
                        ...
                        <div>
                            ...
                            <textarea id="meta_description" name="meta_description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">{{ old('meta_description') }}</textarea>
